...
/* 	function searchname(id){
	...
	$.ajax({
		type: 'post',
		url: '../include/appajax.php',
		data: {expenseheads:'expenseheads'},
		dataType: 'json',
		success: function(results){
			$("#purpose").append("<option value = '' selected></option>");
			for(var result in results){
				//console.log(results[result].title)
				$("#purpose").append("<option value = '" + results[result].expno + "'>"+ results[result].exphead +"</option>")
			}
		}
	});
	$(".select-box").select2({
