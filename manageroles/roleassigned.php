	...
	<head>
		...
		<script>
			$(document).ready(function(){
				...
				function loadassignedsubmenu(roleid){
					$.ajax({
						type: 'POST',
						url:  '../include/appajax.php',
						data: {loadassignedsubmenu:roleid},
				...
				$('.gosub').live('click',function(){
					...
					function unassign(submenu){
						$.ajax({
							type:'post',
							url:'../include/appajax.php',
							data: {unassignsubmenu:submenu, unassignroleid:$('#rolename').val()},
							success:function(result){
								alert(result);
								//console.log(result);
								submenu = [];
