<?php
	...
	if (isset($_POST['rcptfrmothers']))
	{
		...
		if($error == ''){
			extract($_POST);
			if( cashflowininsert($conn, 0, $receiptno, $receiptdate, '', $amount, $instrument, $refno, $refdate, $bank, $purpose, $payer) == true)
				{
				echo "<form method = 'post' action = 'rcptfrmothers.php' id = 'myForm'><input type = 'hidden' value = 'Successful' name = 'comment' /></form>";
			...
			//echo $error;
			echo "<form method = 'post' action = 'rcptfrmothers.php' id = 'myForm'><textarea name = 'error' >$error</textarea></form>";
			echo "<script>document.getElementById('myForm').submit();</script>";
		}
	}
	else{
		echo "<form method = 'post' action = 'rcptfrmothers.php' id = 'myForm'><input type = 'hidden' value = 'Not successful' name = 'comment'></form>";
		echo "<script>document.getElementById('myForm').submit();</script>";
	}
?>